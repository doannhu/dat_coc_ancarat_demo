...
export function ManufacturerOrderList() {
    ...
    const pendingProductTypeCounts = pendingProducts.reduce((acc, p) => {
        acc[p.product_type] = (acc[p.product_type] || 0) + 1;
        return acc;
    }, {} as Record<string, number>);

    const pendingProductTypeSummary = Object.entries(pendingProductTypeCounts)
                    ...
                    <CardContent>
                        {loading ? (
                            ...
                            <div className="space-y-4">
                                {orders.map((order) => {
                                    const productTypeCounts = order.items.reduce((acc, item) => {
                                        const type = item.product?.product_type || 'Unknown';
                                        acc[type] = (acc[type] || 0) + 1;
                                        return acc;
                                    }, {} as Record<string, number>);

                                    const productSummary = Object.entries(productTypeCounts)
                                        .map(([type, count]) => `${count} x ${type}`)
                                        .join(', ');
                                            ...
                                            >
                                                <div className="flex items-center gap-3">
                                                    <div className="text-gray-500">
                                                        {expandedOrders.has(order.id) ? <ChevronDown className="h-5 w-5" /> : <ChevronRight className="h-5 w-5" />}
                                                    </div>
                                                    <div>
